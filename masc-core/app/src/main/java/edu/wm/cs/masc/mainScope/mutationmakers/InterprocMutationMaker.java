...
import edu.wm.cs.masc.mutation.operators.restrictive.interprocoperator.InterprocNestedConditional;
...
public class InterprocMutationMaker extends AMultiClassMutationMaker {
    ...
    public InterprocMutationMaker(InterprocProperties p) {
        ...
        /*if (p.getBuilder() == "InterProcOperator"){
            FilePack filePack = new FilePack(otherClass, p.getOutputDir(), BuilderNestedConditionalIterations.getInterprocClassString(p));
            filePacks.add(filePack);
        }
        if (p.getBuilder() == "InterProcAddition"){
            FilePack filePack = new FilePack(otherClass, p.getOutputDir(),
                    BuilderInterprocClass.getInterprocClassString(p));
            filePacks.add(filePack);
        }
        if (p.getBuilder() == "InterprocConditional"){
            FilePack filePack = new FilePack(otherClass, p.getOutputDir(),
                    BuilderInterprocClass.getInterprocClassString(p));
            filePacks.add(filePack);
        }
        if (p.getBuilder() == "BaseCaseSeperateClass"){
            FilePack filePack = new FilePack(otherClass, p.getOutputDir(),
                    BuilderInterprocClass.getInterprocClassString(p));
            filePacks.add(filePack);
        }
        if (p.getBuilder() == "InterprocNestedClass"){
            FilePack filePack = new FilePack(otherClass, p.getOutputDir(),
                    BuilderNestedConditionalIterations.getInterprocClassString(p));
            filePacks.add(filePack);
        }*/
        //System.out.println(p.getBuilder());
